...
RUN yum update -y && yum install -y autoconf automake bc bison bzip2 flex fontconfig freetype gcc-c++ git glibc-devel glibc-devel.i686 java-1.8.0-openjdk-devel libgcc.i686 libstdc++.i686 m4 make mercurial openssl-devel patch perl-ExtUtils-MakeMaker rsync squashfs-tools subversion unzip wget which && yum clean all
